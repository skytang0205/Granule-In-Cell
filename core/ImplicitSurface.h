...
namespace Pivot {
    ...
    public:
        ...
        virtual Vector2d ClosestPositionOf(Vector2d const & pos) const override {
            return m_Center + ClosestNormalOf(pos) * m_Radius;
        }
    ...
    public:
        ImplicitBox(Vector2d const & minCorner, Vector2d const & lengths):
            m_Center { minCorner + lengths / 2 }, m_HalfLengths { lengths / 2 } {}
    ...
    public:
        ImplicitPlane(Vector2d const & position, Vector2d const & direction):
            m_Position(position), m_Normal(direction.normalized()) {}
        ...
        virtual std::pair<Vector2d, Vector2d> GetCornersOfAABB() const override { return { Vector2d::Zero(), Vector2d::Zero() }; }
    ...
    public:
        ...
        virtual Vector2d ClosestPositionOf(Vector2d const & pos) const override {
            return pos / pos.cwiseQuotient(m_SemiAxels).norm();
        } // not accurate solution
        virtual Vector2d ClosestNormalOf(Vector2d const & pos) const override {
            return (pos - ClosestPositionOf(pos)).normalized() * (Surrounds(pos) ? -1 : 1);
        }
        virtual double DistanceTo(Vector2d const & pos) const override { return (pos - ClosestPositionOf(pos)).norm(); }
        virtual double SignedDistanceTo(Vector2d const & pos) const override {
            return DistanceTo(pos) * (Surrounds(pos) ? -1 : 1);
        }
