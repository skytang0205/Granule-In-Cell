...
namespace Pivot {
    ...
    public:
        StaggeredGrid(int boundaryWidth, double spacing, Vector2i resolution, Vector2d center = Vector2d::Zero()):
            m_BoundaryWidth { boundaryWidth }, m_Spacing { spacing }, m_InvSpacing { 1 / m_Spacing }, m_Resolution { resolution },
            m_Origin { center - m_Resolution.cast<double>() * m_Spacing / 2 },
            m_NodeGrid(m_Spacing, m_Resolution + Vector2i::Ones(), m_Origin),
            m_CellGrid(m_Spacing, m_Resolution, m_Origin + Vector2d::Constant(m_Spacing / 2)), m_FaceGrids {
        ...
        Grid CreateRefinedCellGrid(int factor) const {
            return Grid(m_Spacing / factor, m_Resolution * factor, m_Origin + Vector2d::Constant(m_Spacing / factor / 2));
        }

        Vector2d GetDomainOrigin() const { return m_Origin + Vector2d::Constant(m_BoundaryWidth * m_Spacing); }
        Vector2d GetDomainLengths() const {
            return (m_Resolution - Vector2i::Constant(m_BoundaryWidth * 2)).cast<double>() * m_Spacing;
        }
        ...
        bool IsBoundaryFace(int axis, Vector2i const & face) const {
            return face[axis] <= m_BoundaryWidth || face[axis] >= m_Resolution[axis] - m_BoundaryWidth
                || ! IsInsideFace(axis, face);
        }
        ...
        static std::pair<int, Vector2i> FaceOfCell(Vector2i const & cell, int ord) {
            return { FaceAxisOfCell(ord), cell + Vector2i::Unit(FaceAxisOfCell(ord)) * (ord & 1) };
        }
        static Vector2i AdjCellOfFace(int axis, Vector2i const & face, int ord) {
            return face - Vector2i::Unit(axis) * (ord & 1 ^ 1);
        }

        static constexpr int            EdgeAxisOfCell(int ord) { return ord >> 1; }
        static std::pair<int, Vector2i> EdgeOfCell(Vector2i const & cell, int ord) {
            return { EdgeAxisOfCell(ord), cell + Vector2i::Unit(EdgeAxisOfCell(ord) ^ 1) * (ord & 1) };
        }

        static Vector2i NodeOfCell(Vector2i const & cell, int ord) { return cell + Vector2i(ord & 1, ord >> 1 & 1); }
        static Vector2i NodeOfFace(int axis, Vector2i const & face, int ord) {
            return face + Vector2i::Unit(axis ^ 1) * (ord & 1);
        }
