...
namespace Pivot {
    ...
    public:
        ...
        {
            return std::max(m_Datas[0].GetMaxAbsValue(), m_Datas[1].GetMaxAbsValue());
        }

        void SetConstant(Vector2<Type> const & value) {
            for (int axis = 0; axis < 2; axis++) m_Datas[axis].SetConstant(value[axis]);
        }
