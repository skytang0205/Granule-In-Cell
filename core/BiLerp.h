...
namespace Pivot {
    ...
    public:
        static std::array<Vector2i, 4> GetPoints(Grid const & grid, Vector2d const & pos) {
            ...
            return { lower + Vector2i(0, 0), lower + Vector2i(1, 0), lower + Vector2i(0, 1), lower + Vector2i(1, 1) };
        }
        ...
        template<typename Type> static Type Interpolate(GridData<Type> const & grData, Vector2d const & pos) {
            Type val = Zero<Type>();
            for (auto const [coord, weight] : GetWtPoints(grData.GetGrid(), pos)) { val += grData.At(coord) * weight; }
            return val;
        }
