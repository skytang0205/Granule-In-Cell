...
namespace Pivot {
    ...
    public:
        ...
        static void Solve(GridData<double> & grData, double clearVal, int maxSteps, Func && isValid) {
            ...
            ParallelForEach(grData.GetGrid(), [&](Vector2i const & coord) { valid[coord] = isValid(coord); });
        ...
        static void Solve(SGridData<double> & sgrData, double clearVal, int maxSteps, Func && isValid) {
            ...
            ParallelForEach(
                sgrData.GetGrids(), [&](int axis, Vector2i const & face) { valid[axis][face] = isValid(axis, face); });
            tbb::parallel_for(0, 2, [&](int axis) { Solve(sgrData[axis], clearVal, maxSteps, valid[axis]); });
        }
    };
}
