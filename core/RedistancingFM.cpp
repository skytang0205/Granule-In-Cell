...
namespace Pivot {
    ...
    void RedistancingFM::Solve(GridData<double> & phi, int maxSteps) {
        ...
        ForEach(phi.GetGrid(), [&](Vector2i const & coord) {
            ...
            for (int i = 0; i < Grid::GetNumNeighbors(); i++) {
                ...
                if (phi.GetGrid().IsValid(nbCoord) && phi[coord] * phi[nbCoord] <= 0) {
                    ...
                    tempPhi[axis] =
                        std::min(tempPhi[axis], phi[coord] / (phi[coord] - phi[nbCoord]) * phi.GetGrid().GetSpacing());
                }
            }
        ...
        for (auto const index : intfIndices) { UpdateNeighbors(tent.GetGrid().CoordOf(index), visited, tent, heap); }
        while (! heap.empty()) {
        ...
        ParallelForEach(phi.GetGrid(), [&](Vector2i const & coord) { phi[coord] = (phi[coord] <= 0 ? -1 : 1) * tent[coord]; });
    }

    void RedistancingFM::UpdateNeighbors(
        Vector2i const & coord, GridData<std::int8_t> const & visited, GridData<double> & tent, Heap & heap) {
        for (int i = 0; i < Grid::GetNumNeighbors(); i++) {
    ...
    double RedistancingFM::SolveEikonalEquation(
        Vector2i const & coord, GridData<std::int8_t> const & visited, GridData<double> const & tent) {
        Vector2d tempPhi = Vector2d::Ones() * std::numeric_limits<double>::infinity();
