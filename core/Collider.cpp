...
namespace Pivot {
    Collider::Collider(StaggeredGrid const & sgrid):
        LevelSet(sgrid.GetNodeGrid()), Velocity(sgrid.GetFaceGrids()),
        m_DomainBox(sgrid.GetDomainOrigin(), sgrid.GetDomainLengths()), m_Fraction(sgrid.GetFaceGrids()),
        m_Normal(sgrid.GetFaceGrids()), m_AuxLevelSet(sgrid.GetCellGrid()) {
    ...
    void Collider::Finish(StaggeredGrid const & sgrid) {
        ...
        auto levelSetDrvs =
            std::to_array({ GridData<double>(sgrid.GetNodeGrid()), GridData<double>(sgrid.GetNodeGrid()) });
    ...
    void Collider::Enforce(std::vector<DEMParticle> & particles, double dt, double radius) const {
        tbb::parallel_for_each(particles.begin(), particles.end(), [&](DEMParticle & particle) {
            double const phi = BiLerp::Interpolate(LevelSet, particle.Position);
            if (phi < radius * 1.05) {
                Vector2d const delta_vel = particle.Velocity - BiLerp::Interpolate(Velocity, particle.Position);
                Vector2d const n         = BiLerp::Interpolate(m_Normal, particle.Position).normalized();
                if (phi < 0) {
                    particle.Position -= n * (phi);
                    if (delta_vel.dot(n) < 0) particle.Velocity -= 1.5 * delta_vel.dot(n) * n;
                }
                if (particle.AccVelocity.dot(n) < 0)
                    particle.AccVelocity += particle.AccVelocity.dot(n) * std::tan(DEMParticle::FricAngle)
                        * (delta_vel - delta_vel.dot(n) * n).normalized();
            }
        });
    }
} // namespace Pivot
